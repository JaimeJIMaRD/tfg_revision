                    ...
                    <div class="grid grid-cols-5 py-5 w-fit text-center" id="pag{{ $index + 1 }}">
                        @foreach ($chunk as $personaje)
                            @php
                                ...
                                if($file) {
$imagen_fondo_url = url($file->name);
                                } else {
                                    $imagen_fondo_url = asset('https://static.vecteezy.com/system/resources/thumbnails/005/129/844/small_2x/profile-user-icon-isolated-on-white-background-eps10-free-vector.jpg');
                                }
                            @endphp
                            ...
                            <div class="bg-gray-400 w-24 h-24 m-1.5 hover:scale-105 transition-transform duration-100 ease-in-out rounded-sm casilla{{ $loop->parent->index * 16 + $loop->index + 1 }}"
                                 ...
                                 style="background-size: 100%; background-position: center; background-image: url('/peticiones/{{ $imagen_fondo_url }}');"
                            >
                            </div>
